...
class FinalBlock(nn.Module):
    def __init__(self,in_channels):
        ...
        self.final_conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)  # Refinement layer
        
    def forward(self, x):
        x = self.downsample(x)  # From (448, 448) to (224, 224)
        x = self.final_conv(x)  # Keeps size same (optional)
        return x
...
class UpblockAttention(nn.Module):
    def __init__(self, in_channels, out_channels, nb_Conv, activation='ReLU'):
        ...
        self.pixModule = SCDMModule(in_channels // 2)
        self.nConvs = _make_nConv(in_channels, out_channels, nb_Conv, activation)

    def forward(self, x, y, z):
        up = self.up(x)
        skip_x_att = self.pixModule(x,y,z)
        x = torch.cat([skip_x_att, up], dim=1)  # dim 1 is the channel dimension
        return self.nConvs(x)


class LLMTAMIS(nn.Module):
    def __init__(self, config, n_channels=3, n_classes=1, img_size=224, vis=False):
        ...
        self.final_block = FinalBlock(in_channels)
        ...
        self.scdm_module1 = SCDMModule(64)
    ...
    def forward(self, x, text):
        ...
        z1 = self.reconstruct1(y1) + x1
        z2 = self.reconstruct2(y2) + x2
        z3 = self.reconstruct3(y3) + x3
        z4 = self.reconstruct4(y4) + x4
        # print(x5.size(), x4.size(),x3.size())
